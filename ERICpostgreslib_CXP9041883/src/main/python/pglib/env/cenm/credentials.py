...
class PostgresUserEncryptedPasswordCenm(EnmUserEncryptedPasswordBase):
    ...
    @cached_property()
    def password(self):
        status, password = self.shell.run('kubectl get secret '
                                          'document-database-pg-secret -o '
                                          'jsonpath={.data.super-pwd} | '
                                          'base64 --decode')
        if password and status == 0:
            return password
        return super(PostgresUserEncryptedPasswordCenm, self).password
